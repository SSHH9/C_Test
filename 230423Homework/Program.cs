...
namespace _230423Homework
{
    struct Pokemon
    {
        ...
        public Pokemon(int number, string name,string type, double height, double weight, double attack, double defense, double speed)
        {
            this.number = number;
            this.name = name;
            this.type = type;
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var aData4 =
                from data in arrPokemon
                where data.height > 0.5 && data.weight >= 6
                select data.name;
