using System;
using System.Collections.Generic;

namespace _230212Assignment10_280
{
    class Account
    {
        public string name;
        public float Money;
        public int count;
        public string accountNum;
        public int inputcount = 0;

        List<float> deposit_log = new List<float>() { };
        List<float> withdraw_log = new List<float>() { };

        public Account(string name, float Money)
        {
            this.name = name;
            this.Money = Money;

            Random rand = new Random();
            string a1 = rand.Next(100, 1000).ToString();
            string a2 = rand.Next(10, 100).ToString();
            string a3 = rand.Next(100000, 1000000).ToString();

            this.accountNum = a1 + "-" + a2 + "-" + a3;

            //Console.WriteLine("Bank: SC Bank");
            //Console.WriteLine("Account: {0}", accountNum);
        }

        public void get_account_num()
        {
            Console.WriteLine();
        }
        public void deposit(float deposit)
        {
            if (deposit >= 1)
            {
                Money += deposit;

                inputcount++;

                if (inputcount % 5 == 0)
                {
                    Money = Money * 1.01f;
                }
                deposit_log.Add(deposit);
            }
        }
        public void withdraw(float withdraw)
        {
            if (Money > withdraw)
            {
                Money -= withdraw;

                withdraw_log.Add(withdraw);
            }

        }
        public void display_info()
        {
            Console.WriteLine("Bank: SC Bank");
            Console.WriteLine("Name: {0}", name);
            Console.WriteLine("Account: {0}", accountNum);
            Console.WriteLine("Balance: {0}", Money);
        }
        public void deposit_history()
        {
            foreach (var item in deposit_log)
            {
                Console.WriteLine(item);
            }
        }
        public void withdraw_history()
        {
            foreach (var item in withdraw_log)
            {
                Console.WriteLine(item);
            }
        }
    }
    class Program
    {
        static void Main(string[] args)
        {
            Account account = new Account("python", 0);

            //    account.deposit(1000);
            //    account.withdraw(100);

            //    List<Account> list = new List<Account>() {
            //        new Account("Kim", 10000000),
            //        new Account("Lee", 10000),
            //        new Account("Park", 10000),
            //};
            //    foreach (var item in list)
            //    {
            //        if (item.Money >= 1000000)
            //        {
            //            item.display_info();
            //        }
            //    }

            account.deposit(100);
            account.deposit(200);
            account.deposit(300);
            account.deposit_history();

            account.withdraw(100);
            account.withdraw(200);
            account.withdraw_history();
        }
    }
}
